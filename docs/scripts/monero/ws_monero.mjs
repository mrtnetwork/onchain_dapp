...
async function connect() {
    ...
    let { accounts } = await provider.features["monero:connect"].connect(network);
...
async function requestAccounts() {
    ...
    await utils.runMethod({
        ...
        asyncFunc: async function name() {
            const { accounts } = await provider.features["monero:connect"].connect();
...
async function signMessage() {
    ...
    await utils.runMethod({
        ...
        asyncFunc: async function name() {
            const { signature } = await provider.features["monero:signMessage"].signMessage(params);
...
async function signAndSendTransaction() {
    ...
    await utils.runMethod({
        ...
        asyncFunc: async function name() {
            const response = await provider.features["monero:signAndSendTransaction"].signAndSendTransaction(params);
            return response;
        }
    })
}
async function disconnect() {
    ...
    await utils.runMethod({
        method: "disconnect",
        asyncFunc: async function name() {
            const disconnect = await provider.features["monero:disconnect"].disconnect();
            return disconnect;
        }
    })
}
...
async function listenOnNetworkChanges() {
    const { provider } = await connect();
    await provider.features["monero:events"].on("change", function name({ accounts, chains }) {
        if (accounts) {
