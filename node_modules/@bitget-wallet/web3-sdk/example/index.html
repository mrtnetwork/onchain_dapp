<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, user-scalable=no">
    <title>web3-sdk</title>
    <script src="../dist/index.iife.js"></script>
</head>

<body>
    <div id="WalletInfo">
        <img style="width:30px" src="data:image/svg+xml;base64,BitgetWallet" alt="">
        <a href=''></a>


    </div>
    <script type='module'>
        import { detectBitkeepProvider, TronLinkAdapter, EIP1193Adapter, SolanaAdapter, currentProvider, installWalletMessage, getDownload } from "../dist/index.es.js"
        //get App data
        (() => {
            const sdkAdapter = new EIP1193Adapter()
            // const sdkAdapter = new TronLinkAdapter()
            // const sdkAdapter = new SolanaAdapter()
            const deepLinks = sdkAdapter.getDeepLinks()
            const logo = sdkAdapter.getLogo()
            const WalletInfo = sdkAdapter.getWalletInfo()
            const download = sdkAdapter.getDownloadLinks()

            const appNameLink = document.querySelector("#WalletInfo a")
            const appLogo = document.querySelector("#WalletInfo img")
            appNameLink.href = WalletInfo.homepage
            appNameLink.innerText = WalletInfo.name
            appLogo.src = WalletInfo.logo

            console.log('download', {
                deepLinks,
                logo,
                WalletInfo,
                download
            })
        })();


        //get provider 
        (async function () {
            const sdkEIP1193Adapter = new EIP1193Adapter()
            const Etherume = sdkEIP1193Adapter.getProvider()

            console.log('getProvider', Etherume)

            if (Etherume) {
                await Etherume.request({ method: 'eth_requestAccounts' })
                console.log("Etherume----- html", await Etherume.request({ method: 'eth_accounts' }), sdkEIP1193Adapter.getWalletInfo())
            }

            const sdkTronLinkAdapter = new TronLinkAdapter()
            const tronWallet = sdkTronLinkAdapter.getProvider()
            if (tronWallet) {
                const { tronLink, tronWeb } = tronWallet
                await tronLink.request({ method: 'tron_requestAccounts' })
                console.log("tronLink-----", tronWeb.defaultAddress.hex)
            }

            const sdkSolanaAdapter = new SolanaAdapter()
            const solanaProdivder = sdkSolanaAdapter.getProvider()
            if (solanaProdivder) {
                await solanaProdivder.connect()
                console.log("solana-----", solanaProdivder.publicKey.toString())
            }
        })();

        // detected
        (async function () {
            const Etherume = await detectBitkeepProvider({ flag: "eip1193", timeout: 1000 })

            console.log(Etherume, 'detectBitkeepProvider')
            if (Etherume) {
                await Etherume.request({ method: 'eth_requestAccounts' })
                console.log("eip1193----", await Etherume.request({ method: 'eth_accounts' }))
            }

            const tronProvider = await detectBitkeepProvider({ flag: "tronLink", timeout: 1000 })
            if (tronProvider) {
                const { tronLink } = tronProvider
                await tronProvider.tronLink.request({ method: 'tron_requestAccounts' })
                console.log("tronLink------", tronProvider.tronWeb.defaultAddress.hex)
            }

            const solanaProdivder = await detectBitkeepProvider({ flag: "solana", timeout: 1000 })
            if (solanaProdivder) {
                await solanaProdivder.connect()
                console.log("solana------", solanaProdivder.publicKey.toString())
            }
        })();

        (async function () {
            console.log(currentProvider(), 'currentProvider')
            console.log(installWalletMessage, 'installWalletMessage', getDownload())

            const sdkAdapter = new EIP1193Adapter()
            const WalletInfo = sdkAdapter.getWalletInfo()
            console.log(WalletInfo, 'getWalletInfo')
            console.log(WalletInfo.logolist.svg[256],'iconBase64')
        })();



    </script>
</body>

</html>