...
async function signTypedData() {
    ...
    console.log("is called: " + accounts[0].address)
    const params = {
        account: accounts[0],
...
async function subscribe() {
    const wallet = await getWallet();
    try {
        await wallet.features["ethereum:events"].on('accountsChanged',
            (p) => console.log("message in page: " + JSON.stringify(p))
        );
    } catch (e) {
    ...
    // };
    // let subId = await wallet.features["ethereum:request"].request(params);
